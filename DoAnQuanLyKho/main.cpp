...
int main()
{
	...
	while (true)
	{
		...
		{
			system("cls");
			string choice; vector<mathang*> lmh; string tenmathang, tenloaimathang, mahoadon; bool check1 = true, check2 = true;
			int vitriloaimathang, vitrimathang, dongia = 0; vector<int> VITRILOAIMATHANG; vector<int> VITRIMATHANG;
			...
			while (true)
			{
				...
				do {
					...
					else if (choice == "0")
					{
						...
						for (int i = 0; i < lmh.size() - 1; i++)
						{
							Fileout << "\n" << mahoadon << setw(m - mahoadon.length() + 30 - t + lmh[i]->Get_tenmh().length()) << lmh[i]->Get_tenmh()
								<< setw(t - lmh[i]->Get_tenmh().length() + 30 - s + lmh[i]->Get_soluongmh().length())
								<< lmh[i]->Get_soluongmh() << setw(s - lmh[i]->Get_soluongmh().length() + 30 - d + lmh[i]->Get_DonGia().length()) << lmh[i]->Get_DonGia()
								<< setw(d - lmh[i]->Get_DonGia().length() + 30 - nx + curdate.length()) << curdate << "\n";
						}
						Fileout << "\n" << mahoadon << setw(m - mahoadon.length() + 30 - t + lmh[lmh.size() - 1]->Get_tenmh().length()) << lmh[lmh.size() - 1]->Get_tenmh()
		...
		{
			string choice; vector <int> VITRILOAIMATHANG1; int dongia1 = 0, dongia2 = 0; bool check1 = true, check2 = true;
			vector<mathang*> lmh; vector<int> VITRILOAIMATHANG2; vector<int> VITRIMATHANG2; string mahoadon, ngaynhap; 
			vector<int> VITRITONG; vector<loaimathang*> LMH;
			system("cls");
			...
			while (true)
			{
							...
							if (choice2 == "1")
							{
								...
								do {
									...
									else
									{
										...
										VITRITONG.push_back(vitriloaimathang);
									}
									system("pause");
								} while (check1 == false || check2 == false);
							}
				...
				{
					if (lmh.size() == 0 && LMH.size() == 0) break;
					ofstream fileout("hoadonnhaphang.txt", ios_base::app);
					if (MAHOADONNHAP.size() == 0) fileout << mahoadon << setw(20) << curdate;
					else fileout << "\n" << mahoadon << setw(20) << curdate;
					fileout.close();
					string tenfile = "ChiTietHoaDonNhapMaHoaDon_" + mahoadon + ".txt";
					ofstream Fileout(tenfile);
					if (lmh.size() != 0)
					{
		...
		else if (luachon == "9")
		{
			...
			while (true)
			{
				...
				cout << "\n\t\t\t\t\t\t\t\t\t\t\t0. Thoat\n";
				string choice4; string s = "12340"; char c; bool check = true; 
				do 
				{
					check = false;
					mathang::NhapChuoi("\n\t\t\t\t\t\t\t\t\t\t\tNhap vao lua chon: ", choice4);
				...
				if (choice4 != "0") List.ThongKe(choice4, vitriloaimathanghethan, tenmathanghethan, demTK4);
				else break;
			}
		}
		else if (luachon == "0")
